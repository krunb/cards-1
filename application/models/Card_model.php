...
class Card_model extends CI_Model {
	...
	function get_all_cards($per_page, $page){
		return $this->db 
		->limit($per_page, $page)
...
*/ 
	...
	function get_card_item_id($card_item_id){
	    return $this->db->get_where("card_item","card_item_id = $card_item_id")->row();
	}
	function update_card_item($post){
		$arr = array(
			"card_item_code"		=> $post["card_item_code"],
			"card_item_reference"	=> $post["card_item_reference"],
			"card_item_serial"		=> $post["card_item_serial"],
		...
		return $this->db->where("card_item_id", $post["card_item_id"])->update("card_item", $arr); 
	}
}