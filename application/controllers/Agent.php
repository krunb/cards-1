...
class Agent extends CI_Controller {
	...
	public function profile(){
		...
		$data["page"] = "back/agent/profile";
		$this->load->view('include/temp',$data);
	}
	
	public function wallet(){
		$config = array();
        $config["base_url"] 	= base_url() . "agent/wallet";
        $config["total_rows"] 	= $this->agent_model->get_count();
        $config["per_page"] 	= 20;
		...
		$data["view"] = $this->agent_model->get_all_wallet($config["per_page"], $page);
		$data["customer"] 	= $this->customer_model->get_all_customer();
		$data["page"] = "back/agent/wallet/index";
		$this->load->view('include/temp',$data); 
	} 
	...
	public function charge_wallet(){
		...
		$data["customer"] 	= $this->customer_model->get_all_customer();
		$data["page"] = "back/agent/wallet/charge_wallet";
		$this->load->view('include/temp',$data); 
	}
	...
	public function customer_wallet(){
		...
		if($customer_id != "" and $customer_id > 0){
			$data["view"] = $this->agent_model->customer_wallet($customer_id); 
			$this->load->view("back/agent/wallet/customer_wallet", $data); 
		} else {
	...
	public function do_charge_wallet(){
		...
		$wallet_id = $this->agent_model->do_charge_wallet($post, $bank_receipt, $old_balance->customer_balance);
